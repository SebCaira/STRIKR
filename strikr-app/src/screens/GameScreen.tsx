...
export default function GameScreen({ onBack }: { onBack?: () => void } = {}) {
  ...
  const [resultSnapshot, setResultSnapshot] = useState<{ player: NonNullable<typeof state.player>; solvedAt: number; level: Level; winStreak: number; reward: number; rewardCapped: boolean } | null>(null);
  ...
  }, [state.status, state.player, state.solvedAt, state.level, state.winStreak, state.lastReward, state.rewardCapped]);
