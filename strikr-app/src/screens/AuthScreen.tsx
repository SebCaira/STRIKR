...
export default function AuthScreen() {
  ...
  const submitForgotEmail = async () => {
    ...
    setBusy(false);
    if (sendError) {
      setError(friendlyError(sendError, t('error_generic')));
      return;
    }
    setMode('forgot-code');
  };
  ...
  const submitResetCode = async () => {
    ...
    if (verifyError) {
      setBusy(false);
      setError(friendlyError(verifyError, t('error_generic')));
      return;
    }
    const { error: updateError } = await supabase.auth.updateUser({ password: newPassword });
    setBusy(false);
    if (updateError) {
      setError(friendlyError(updateError, t('error_generic')));
      return;
    }
    setResetDone(true);
  };
  ...
  const submit = async () => {
    ...
    if (mode === 'signup') {
      ...
      if (checkError) {
        setBusy(false);
        setError(friendlyError(checkError, t('error_generic')));
        return;
      }
      if (taken) {
        setBusy(false);
    ...
    // A signed-in user can set a real recovery email (Settings), which
    // Supabase then uses as the account's actual auth email — so sign-in
    // must look up whatever email is currently on file rather than always
    // assuming the synthetic one, or the account becomes unreachable by
    // username once a recovery email is confirmed.
    let email = usernameToEmail(cleanUsername);
    if (mode === 'signin') {
      const { data: realEmail } = await supabase.rpc('email_for_username', { check_username: cleanUsername });
      if (realEmail) email = realEmail;
    }
