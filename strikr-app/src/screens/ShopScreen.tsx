...
import { SHOP_PACKAGES, REWARDED_AD_DIAMONDS, REWARDED_AD_COOLDOWN_MS } from '../data/shop';
import HardShadowBox from '../components/HardShadowBox';

const AD_COOLDOWN_KEY = 'strikr_ad_cooldown_until_v1';
...
export default function ShopScreen() {
  ...
  const [adCooldownUntil, setAdCooldownUntil] = useState<number | null>(null);
  const [adWatching, setAdWatching] = useState(false);
  const [now, setNow] = useState(Date.now());

  useEffect(() => {
    AsyncStorage.getItem(AD_COOLDOWN_KEY).then((raw) => {
      if (raw) setAdCooldownUntil(Number(raw));
    });
  }, []);
  ...
  const adRemainingMs = adCooldownUntil ? adCooldownUntil - now : 0;
  const adReady = adRemainingMs <= 0;

  const watchAd = useCallback(() => {
    if (!adReady || adWatching) return;
    setAdWatching(true);
    // The cooldown starts immediately (not after the simulated delay) and is
    // persisted right away, so leaving and re-entering the screen mid-delay
    // can't be used to restart the timer and double up the reward.
    const until = Date.now() + REWARDED_AD_COOLDOWN_MS;
    setAdCooldownUntil(until);
    AsyncStorage.setItem(AD_COOLDOWN_KEY, String(until)).catch(() => {});
    setTimeout(() => {
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ fontFamily: fonts.body, fontSize: 11, color: 'rgba(255,255,255,.7)', marginTop: 2 }}>
                {adReady ? t('shop_ad_sub') : `${t('shop_ad_cooldown_prefix')} ${formatCountdown(adRemainingMs)}`}
              </Text>
            </View>
          </Pressable>
        </HardShadowBox>
