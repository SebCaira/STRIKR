...
import { GAME_WIN_XP, HINT_COSTS, Level, NAT_FR, POS_FR, flagUrl, rewardFor } from '../game/engine';
...
export default function GameScreen() {
  ...
  const [resultSnapshot, setResultSnapshot] = useState<{ player: NonNullable<typeof state.player>; solvedAt: number; level: Level } | null>(null);
  ...
  useEffect(() => {
    if ((state.status === 'won' || state.status === 'lost') && state.player) {
      setResultSnapshot({ player: state.player, solvedAt: state.solvedAt || 1, level: state.level || 'medium' });
    }
  }, [state.status, state.player, state.solvedAt, state.level]);
