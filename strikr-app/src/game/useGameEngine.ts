  ...
  const buyLetterHint = useCallback(
    (kind: LetterHintKind) => {
      setState((prev) => {
        ...
        const next =
          kind === 'placed'
            ? revealPlacedLetter(prev.player.n, prev.letterHints)
            : revealPresentLetter(prev.player.n, prev.letterHints);
        if (next === prev.letterHints) return prev; // nothing left to reveal
        return { ...prev, letterHints: next, pendingEffect: { kind: 'hint', cost } };
      });
    },
    [diamonds, flashLowDiamonds]
  );
