...
export default function DuelScreen() {
  ...
  const { duel, loading, myRole, createDuel, inviteDuel, respondInvite, cancelInvite, joinDuel, playCell, passTurn, forfeit, clearFinished } = useDuel();

  // Came here via LeagueScreen's "challenge" button: send the invite once
  // loading has settled and we know there's no active one already, then
  // clear the params so refocusing this tab later doesn't refire it.
  useEffect(() => {
    if (!route.params?.inviteUserId || loading || duel) return;
    inviteDuel(route.params.inviteUserId, route.params.inviteUserName || '');
    navigation.setParams({ inviteUserId: undefined, inviteUserName: undefined });
  }, [route.params?.inviteUserId, route.params?.inviteUserName, loading, duel, inviteDuel, navigation]);
