...
async function getJSON(url) {
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (requestsLogged < 8) {
        console.log(`  [debug] HTTP ${r.status} for ${url.slice(0, 100)}...`);
        requestsLogged++;
      }
...
// Fetches ALL claims (not filtered server-side by property) and extracts
// P154 client-side — same pattern as build-club-data.js, which reliably
// gets country/founded/capacity this way. An earlier version of this script
// filtered with `&property=P154` server-side and that came back empty for
// clubs that definitely have a logo on Wikidata (Real Madrid, Man City...),
// so this fetches the full claims object instead, like the proven script.
function wbAllClaims(qid) {
...
async function resolveLogo(name) {
  ...
  const claims = await wbAllClaims(hit.id);
  const filename = claimValue(claims, 'P154');
  if (!filename) {
