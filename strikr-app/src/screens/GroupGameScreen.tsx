...
// across "Devine le joueur" (main), "Devine le club" (club), and "Mode
// Liste" (liste) — see useGroupGame for the lobby/sync/reward model this
// sits on top of. Grille keeps its own separate turn-based DuelScreen.
...
export default function GroupGameScreen({ onBack, gameType, variant }: GroupGameScreenProps) {
  ...
  useEffect(() => {
    ...
    finishGame();
  }, [secondsLeft, game?.id, game?.status, solved, gameType, wrongList.length, clubGuesses.length, foundIndexes.length, submitResult, finishGame]);
  ...
  const submitClubGuess = () => {
    if (!mysteryClub || solved || !game || game.status !== 'active' || !answer.trim()) return;
  ...
  if (game.status === 'active') {
    const contentReady = gameType === 'main' ? !!mysteryPlayer : gameType === 'club' ? !!mysteryClub : !!mysteryList;
    if (!contentReady) {
      return withBack(
        <View style={{ flex: 1, backgroundColor: colors.bg, alignItems: 'center', justifyContent: 'center' }}>
          <ActivityIndicator color={colors.ink} />
