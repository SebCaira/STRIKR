...
export default function LeagueScreen() {
  ...
  const { leaderboard, loading: boardLoading } = useLeagueLeaderboard(selectedId);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg, paddingTop: insets.top + 14 }}>
      ...
      {leagues.length > 1 && (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, paddingTop: 10, gap: 6 }}>
          {leagues.map((l) => (
            <Pressable
              key={l.id}
      ...
      {leaguesLoading ? (
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 24, alignItems: 'center' }}>
          ...
          >
            <Text style={{ fontFamily: fonts.display, fontSize: 12, color: '#fff' }}>{t('league_create_or_join')}</Text>
          </Pressable>
        </View>
      ) : boardLoading ? (
        <ActivityIndicator color={colors.muted} style={{ marginTop: 30 }} />
        ...
        <>
          ...
          <ScrollView contentContainerStyle={{ padding: 20 }}>
            ...
            >
              <Text style={{ fontFamily: fonts.displayBold, fontSize: 12, color: colors.ink }}>{t('league_manage_leagues')}</Text>
            </Pressable>
          </ScrollView>
        </>
      )}
