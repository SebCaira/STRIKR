...
export default function LeagueScreen() {
  ...
  // DuelScreen owns the single canonical useDuel() instance (it's a sibling
  // tab that stays mounted) — this just hands it a signal to send the
  // invite, rather than running a second useDuel() here with its own
  // realtime subscription racing the one already live in the Duel tab.
  const challengeFriend = (friend: LeaderboardRow) => {
    navigation.navigate('Duel', { inviteUserId: friend.id, inviteUserName: friend.display_name });
  };
              ...
              {(podium.length === 3 ? rest : leaderboard).map((r: LeaderboardRow, i: number) => {
                ...
                const row = (
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, padding: r.is_you ? 9 : 8 }}>
                    ...
                    <AvatarFrame frameId={r.equipped_frame} size={r.is_you ? 30 : 28}>
