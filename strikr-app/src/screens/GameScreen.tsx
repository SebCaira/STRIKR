...
import { GAME_WIN_XP, HINT_COSTS, LETTER_HINT_COSTS, LetterHints, Level, NAT_FR, placedHintExhausted, POS_FR, presentHintExhausted, flagUrl, rewardFor, streakMultiplier } from '../game/engine';
...
export default function GameScreen() {
  ...
  const { state, diamonds, suggestions, pickPlayer, setGuess, submit, buyHint, buyLetterHint, skipOrForfeit } = useGameEngine();
  const { adVisible, recordRoundPlayed, dismissAd } = useInterstitialAd();
  const { stats } = useStats();
  const [flashDiamonds, setFlashDiamonds] = useState(false);
  const [resultSnapshot, setResultSnapshot] = useState<{ player: NonNullable<typeof state.player>; solvedAt: number; level: Level; winStreak: number } | null>(null);
  ...
  useEffect(() => {
    if ((state.status === 'won' || state.status === 'lost') && state.player) {
      setResultSnapshot({ player: state.player, solvedAt: state.solvedAt || 1, level: state.level || 'medium', winStreak: state.winStreak });
    }
  }, [state.status, state.player, state.solvedAt, state.level, state.winStreak]);
  ...
  const letterPlacedExhausted = player ? placedHintExhausted(player.n, state.letterHints) : true;
  const letterPresentExhausted = player ? presentHintExhausted(player.n, state.letterHints) : true;
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg, paddingTop: insets.top }}>
      ...
      {player && (
        <View style={{ paddingHorizontal: 20, paddingTop: 6 }}>
          ...
          <LetterTiles name={player.n} hints={state.letterHints} />
          <View style={{ flexDirection: 'row', gap: 5, flexWrap: 'wrap', marginTop: 6 }}>
            <LetterHintButton
              ...
              diamonds={diamonds} exhausted={letterPresentExhausted} onPress={() => buyLetterHint('present')}
            />
            <LetterHintButton
              icon="🟩" label={t('hint_letter_placed')} cost={LETTER_HINT_COSTS.placed}
              diamonds={diamonds} exhausted={letterPlacedExhausted} onPress={() => buyLetterHint('placed')}
            />
          </View>
        </View>
      )}
...
function LetterTiles({ name, hints }: { name: string; hints: LetterHints }) {
  const { colors, accent, fonts } = useTheme();
  return (
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 4, marginTop: 8 }}>
      {[...name].map((ch, i) => {
        if (ch === ' ') return <View key={i} style={{ width: 8 }} />;
        if (!/[a-zà-öø-ÿ]/i.test(ch)) {
          return (
            <View key={i} style={{ width: 14, height: 24, alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ fontFamily: fonts.displayBold, fontSize: 13, color: colors.muted }}>{ch}</Text>
            </View>
          );
        }
        const revealed = hints.placed[i];
        return (
          <View
            key={i}
            style={{
              width: 22, height: 26, borderRadius: 5, borderWidth: 1.5, borderColor: colors.border,
              backgroundColor: revealed ? accent.mint : colors.card, alignItems: 'center', justifyContent: 'center',
            }}
          >
            <Text style={{ fontFamily: fonts.displayBold, fontSize: 13, color: colors.ink }}>{revealed || ''}</Text>
          </View>
        );
      })}
      {hints.present.length > 0 && (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 4, marginLeft: 6 }}>
          {hints.present.map((l) => (
            <View key={l} style={{ width: 22, height: 26, borderRadius: 5, borderWidth: 1.5, borderColor: colors.border, backgroundColor: accent.yellow, alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ fontFamily: fonts.displayBold, fontSize: 13, color: '#1a1a1a' }}>{l.toUpperCase()}</Text>
            </View>
          ))}
        </View>
      )}
...
function LetterHintButton({
  icon, label, cost, diamonds, exhausted, onPress,
}: {
  icon: string; label: string; cost: number; diamonds: number; exhausted: boolean; onPress: () => void;
}) {
  const { colors, fonts } = useTheme();
  const afford = diamonds >= cost && !exhausted;
  return (
    <Pressable
      onPress={onPress}
      disabled={!afford}
...
function WinOverlay({
  ...
  const diamondsReward = Math.round(rewardFor(level, solvedAt) * multiplier);
          ...
          <View style={{ paddingHorizontal: 22, paddingTop: 12, flexDirection: 'row', gap: 8 }}>
            <View style={{ flex: 1, backgroundColor: accent.mint, borderWidth: 2.5, borderColor: '#1a1a1a', borderRadius: 12, padding: 10, alignItems: 'center' }}>
              ...
              <Text style={{ fontFamily: fonts.mono, fontSize: 9, color: 'rgba(0,0,0,.55)', marginTop: 2 }}>
                {t('game_gems')}{multiplier > 1 ? ` ×${multiplier}` : ''}
              </Text>
            </View>
