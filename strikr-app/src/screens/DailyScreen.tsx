        ...
        <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <View style={{ alignItems: 'flex-end', gap: 5 }}>
            ...
            <Pressable
              ...
              style={{ width: 22, height: 22, borderRadius: 999, backgroundColor: colors.card, borderWidth: 2, borderColor: colors.border, alignItems: 'center', justifyContent: 'center' }}
            >
      ...
      <Modal visible={legendVisible} transparent animationType="fade" onRequestClose={() => setLegendVisible(false)}>
        ...
        >
          <View style={{ backgroundColor: colors.bg, borderWidth: 2.5, borderColor: colors.border, borderRadius: 20, padding: 22, maxWidth: 320, width: '100%' }}>
            <Text style={{ fontFamily: fonts.display, fontSize: 16, color: colors.ink, marginBottom: 14 }}>{t('daily_legend_title')}</Text>
            <View style={{ gap: 10 }}>
              {(['exact', 'present', 'absent'] as const).map((kind) => (
                <View key={kind} style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                  <View
                    style={{
                      width: 28, height: 28, borderRadius: 6, borderWidth: 2, borderColor: colors.border,
                      backgroundColor: TILE_COLORS[kind].bg,
                    }}
                  />
                  <Text style={{ fontFamily: fonts.bodySemibold, fontSize: 13, color: colors.ink }}>{t(`daily_legend_${kind}`)}</Text>
                </View>
              ))}
            </View>
            <Pressable
              onPress={() => setLegendVisible(false)}
              style={{ marginTop: 18, paddingVertical: 10, backgroundColor: accent.coral, borderRadius: 12, alignItems: 'center' }}
            >
              <Text style={{ fontFamily: fonts.display, fontSize: 13, color: '#fff' }}>{t('daily_legend_close')}</Text>
            </Pressable>
          </View>
        </Pressable>
      </Modal>
