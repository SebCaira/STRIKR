...
export function useGameEngine() {
  ...
  const submit = useCallback(
    (text?: string) => {
      const action: {
        current:
          | { kind: 'win'; reward: number; firstTry: boolean; level: Level; elapsedMs?: number }
          | { kind: 'wrong' }
          | null;
      } = { current: null };
      setState((prev) => {
        ...
        if (matchesGuess(p, guessText)) {
          ...
          action.current = {
            kind: 'win',
            reward: rewardFor(prev.level || 'medium', solvedAt),
            firstTry: prev.wrongList.length === 0,
            level: prev.level || 'medium',
            elapsedMs,
          };
          return { ...prev, status: 'won', solvedAt, revealed: p.clubs.length, guess: '', animateReveal: false };
        }
        action.current = { kind: 'wrong' };
      ...
      if (done?.kind === 'win') {
        addDiamonds(done.reward);
        recordWin({ kind: 'game', firstTry: done.firstTry, xp: GAME_WIN_XP, level: done.level, elapsedMs: done.elapsedMs });
        fx.win();
      } else if (done?.kind === 'wrong') {
        fx.wrong();
      }
    },
  ...
  const skipOrForfeit = useCallback(() => {
    ...
    if (charged) addDiamonds(-FORFEIT_COST);
    else if (short) flashLowDiamonds();
  }, [diamonds, addDiamonds, flashLowDiamonds]);
