...
export default function ProfilScreen() {
  ...
  const friendCount = leaderboard.filter((r) => !r.is_you).length;
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 6, paddingBottom: 24 }}>
        ...
        <View style={{ paddingHorizontal: 20, alignItems: 'center' }}>
          ...
          >
            <Text style={{ fontFamily: fonts.displayBold, fontSize: 11, color: '#fff' }}>👥 {t('profil_my_friends')} · {friendCount}</Text>
          </Pressable>
        </View>
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 14, flexDirection: 'row', gap: 8 }}>
          {[
            { bg: colors.card, val: String(stats.solves), label: t('profil_solves') },
            { bg: accent.mint, val: `🔥 ${stats.currentStreak}`, label: t('profil_streak') },
