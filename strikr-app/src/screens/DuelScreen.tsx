      ...
      <View style={{ padding: 20 }}>
        ...
        {duel.grid.rows.map((r, ri) => (
          <View key={ri} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
            <View style={{ width: 78, paddingRight: 6, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              {r.type === 'club' && <ClubShield name={r.value} size={22} />}
              <Text style={{ flex: 1, fontFamily: fonts.displayBold, fontSize: 10, color: colors.ink }} numberOfLines={2}>{r.label}</Text>
            </View>
